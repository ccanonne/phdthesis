...
%           % 1st plot
%           \addplot [purple!80!white, samples=200, domain=0:6] {0.75*exp(-(x-3)^2)};
          % 2nd plot
          \addplot [NavyBlue!80!white, samples=200, domain=0:6] {0.05*(rnd-0.5)+0.01*sin(cos(x)+rnd*x^5)+0.75*exp(-(x-3)^2)}; 
          
          \node (d) at (axis cs:4,.7) {\color{NavyBlue}$\D$};
          \end{axis}
          \tikzstyle{int}=[draw, decorate, minimum height=5em, minimum width=5em]
          \node[int,circle,dotted] (filter) at (7,1.7) {\; Corrector\; };
          \tikzstyle{int}=[draw, decorate, minimum height=5em, minimum width=5em]
          \node[int,circle] (algo) at (11,1.7) {Algorithm};          
          \begin{axis}[%
              xtick={},
              xticklabels={},
              yticklabels={},
              ...
              scale=0.2,
              name=corrected,
              at={(0.425\linewidth,-0.05\linewidth)}
          ]
          \addplot [draw=none, samples=200, domain=0:6] {0.1*(rnd-0.5)+0.01*sin(cos(x)+rnd*x^5)+0.75*exp(-(x-3)^2)}; 
          \addplot [red!80!white, samples=200, domain=0:6] {0.75*exp(-(x-3)^2)};          
          \node (dtilde) at (axis cs:4.5,.7) {\color{red}$\tilde{\D}$};
          \end{axis}
          
            \path[->] (source.east) edge[decorate] node[above=4pt] {\color{NavyBlue}\scriptsize $s_1,\dots,s_k\sim \D$} (filter.west);
            \path[->] (filter.east) edge[decorate] node[above=4pt] {\color{red}\scriptsize $s_1,\dots,s_m\sim \tilde{\D}$} (algo.west);
\end{tikzpicture}
\caption{\label{fig:sampling:corrector} A sampling corrector acts as a filter being an imperfect source of data $\D$, which is only \emph{close} to having a prespecified property $\property$, and an algorithm which requires data from a source with this property.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
the reader is referred to~\cref{sec:preliminaries:basics} and~\cref{ssec:connections:agnostic}.}
