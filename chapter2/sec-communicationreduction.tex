...
\begin{lemma}\label{lemma:main:methodology:reduction}
	...
	\begin{enumerate}
		...
		\item \emph{Completeness:} For every $(x,y) \in f^{-1}(1)$, it holds that $\p(x,y) \in \Pi$.
		\item \emph{Soundness:} For every $(x,y) \in f^{-1}(0)$, it holds that $\p(x,y)$ is $\eps$-far from $\Pi$ in $\ell_1$ distance.
	\end{enumerate}
