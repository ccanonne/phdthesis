...
However, testing membership in a class using the Fourier transform is significantly more challenging than learning: a fundamental reason being that, in contrast to the learning 
...
We start with some useful structural results, which will be necessary to our analysis. The first is the following lemma from~\cite{DKS:15}:
...
Let $\fourier{\p}$ be the discrete Fourier transform of $\p$ modulo $M$. Then, we have
  ...
  \end{enumerate}
\end{lemma}
Next, we provide a simple structural lemma, bounding the $\lp[2]$ norm of any $(n,k)$-SIIRV as a function of $k$ and its variance only:
...
as desired.
\end{proof}
Finally, we will use the simple fact below -- which follows immediately from~\cite[Claim 2.4]{DKS:15} -- to bound the running time of our algorithm:
...
In this section, we abstract the ideas underlying the $(n,k)$-SIIRV from~\cref{sec:siirv:testing}, to provide a general testing framework. In more detail, our theorem (\cref{theo:testing:general}) has the following flavor: if $\property$ is a property of distributions such that every $\p\in\property$ has both (i) small effective support and (ii) sparse effective Fourier support, then one can test membership in $\property$ with $O(\sqrt{sM}/\eps^2+s/\eps^2)$ samples (where $M$ and $s$ are the bounds on the effective support and effective Fourier support, respectively). As a caveat, we do require that the sparse effective Fourier support $S$ be independent of $\p\in\property$, i.e., is a characteristic of the class $\property$ itself.
...
$ then testing membership in $\classpmd[n]{k}$ requires $\bigOmega{q_{\rm hard}(n,k,\eps)}$ samples as well. This in particular holds for $k = o(n^{c})$ (where e.g. $c<1/9$) and $\eps = 1/2^{O(n)}$.
