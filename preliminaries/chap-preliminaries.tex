...
All throughout this thesis, we denote by $[n]$ the set $\{1,\dots,n\}$, and by $\llbracket n\rrbracket$ the set $\{0,\dots,n-1\}$. We will denote by $\log$  (resp. $\ln$) the binary logarithm (resp. the natural logarithm); we use the notations $\tildeO{f},\tildeOmega{f}$ to hide polylogarithmic dependencies on the argument, and will sometimes write $O_\eps(f)$ to signify that the hidden constant depends on the parameter $\eps$ (while $f$ does not). A \emph{probability distribution} over a (countable) domain\footnote{For the sake of this survey, all distributions will be supported on a finite or at least discrete domain; thus, we do not consider the fully general definitions from measure theory.} $\domain$ is a non-negative function $\D\colon\domain\to[0,1]$ such that $\sum_{x\in\domain} \D(x) = 1$. We denote by $\distribs{\domain}$ the (convex) polytope of all such distributions, and by $\uniformOn{\domain}$ the uniform distribution on \domain (when well-defined). Given a distribution $\D$ over $\domain$ and a set $S\subseteq\domain$, we write $\D(S)$ for the total probability weight $\sum_{x\in S} \D(x)$ assigned to $S$ by $\D$; and let $\supp\D\eqdef\setOfSuchThat{ x\in\Omega }{ \D(x) > 0 }$ be the \emph{(effective) support} of the distribution. Moreover, for $S \subseteq \domain$ such that $\D(S)>0$, we denote by $\D_S$ the conditional distribution of $\D$ restricted to $S$, that is $\D_S(x) = \frac{\D(x)}{\D(S)}$ for $x \in S$ and $\D_S(x)=0$ otherwise. Finally, for a probability distribution $\D\in\distribs{\domain}$ and integer $m$, we write $\D^{\otimes m}\in\distribs{\domain^m}$ for the $m$-fold product distribution obtained by drawing $m$ independent samples $s_1,\dots,s_m\sim \D$ and outputting $(s_1,\dots,s_m)$.
%
% When the domain is a subset of the natural numbers $\N$, we shall identify a distribution $\p\in\distribs{\domain[\Omega]}$ with the sequence $(\p_i)_{i\in\N}\in\lp[1]$ corresponding to its probability mass function (pmf). For a subset $S \subseteq \Omega$, we denote by $\p|_S$ the normalized projection of $\p$ to $S$ (so $\p|_S$ is a probability distribution).
...
As is usual in property testing of distributions, throughout this survey the distance between two distributions $\D_1, \D_2\in \distribs{\domain}$ will be the \emph{total variation distance}:
...
\end{equation}
which takes value in $[0,1]$. In some cases, it is useful to consider (either as a proxy towards total variation, or for the sake of the analysis) different metrics, such as $\lp[2]$, Kolmogorov, Earthmover's or Hellinger distances. More on these can be found in~\autoref{appendix:metrics}.\medskip

A property $\property$ of distributions over $\domain$ is a subset of $\distribs{\domain}$, consisting of all distributions that have the property. The distance from $\D$ to a property $\property$, denoted $\totalvardist{\D}{\property}$, is then defined as $\inf_{\D^\prime \in \property} \totalvardist{\D}{\D^\prime}$.\medskip
...
\end{itemize}
\new{where the probability is taken over the algorithm's randomness and (if any) the randomness from the oracle's answers.}
\end{definition}
We sometimes write $\Tester^{\ORACLE_\D}$ to indicate that $\Tester$ has access to $\ORACLE_\D$. Additionally, we will also be interested in \emph{tolerant} testers -- roughly, algorithms robust to a relaxation of the first item above:
...
\begin{figure}[H]\centering
  \begin{tikzpicture}[decoration={random steps,segment length=1mm,amplitude=1.2pt}, scale=1.5]%[decoration=penciline]
    ...
    \node at (6,2) {$\property$};
  \end{tikzpicture}
  \caption{\label{fig:testing:tolerant:testing}Testing vs. tolerant testing. It is like eggs, only harder.\index{egg}\index{egg|see {testing algorithm (tolerant)}}}%One more ring, and all hell breaks loose.}
\end{figure}
%%%%%%%%%%%%%%%

The last definition we state here is one of \emph{distance estimators}; that is, of algorithms which compute an approximation of the distance of the unknown distribution to a property.
...
We give here the formal descriptions of the classes of distributions involved in this dissertation, starting with that of monotone distributions.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Previous tools from the literature.}
...
Note that the above definition is quite general, and can be instantiated for different types of oracle access to the unknown distribution. In this thesis, we are mainly concerned with two such settings, namely the \emph{sampling oracle access} and \emph{\Cdfsamp oracle access}:
